...
async function findSteps(scheme_id) {
  ...
  */
  const allSteps = await db("steps as st")
    ...
    .where("st.scheme_id", scheme_id) // 2. değişken
    .orderBy("st.step_number", "asc"); //sıralama için
...
async function add(scheme) {
  ...
  */
  const [scheme_id] = await db("schemes").insert(scheme); //insert eklemek için kullanılır
